...
After adapter/invariant tests, the Phase-0 resource gate, the complete prediction pass, coverage calculation, and training-overlap audits for **every learned candidate** (CONTRAfold, LinearPartition-C, and EternaFold)—but **before evaluation code is allowed to read experimental values**—write and checksum `ranked-predictors.json`. A listed candidate enters that frozen ranked set only if its adapter and complete-mass tests pass, Phase 0 passes, every dataset meets the 90% predictor-coverage gates, and every applicable learned-model audit passes. Failures and fixed reasons are retained as secondary results; no replacement predictor is added. Fewer than four ranked predictors or fewer than two represented model families precludes a robust-overall-winner claim.
...
7. The primary overall endpoint is a **study-balanced Spearman score**. First average the condition-level median within-transcript Spearman values within each of four study groups (yeast DMS, mouse icSHAPE, *E. coli* SHAPE, and yeast PARS), giving paired conditions equal weight; then average the four study scores equally. Point estimates retain each condition's complete all-core cohort. For each organism, form one union of normalized-sequence SHA-256 cluster IDs across **all** of that organism's studies and conditions, sample that union once with replacement per replicate, and apply each sampled identity and multiplicity wherever it occurs. This jointly preserves shared-sequence dependence between yeast PARS and yeast DMS as well as within paired conditions; identities appearing in only one dataset contribute only there and are never silently discarded. Organism strata are resampled independently. Each replicate recomputes condition metrics before equal-weight study and four-study aggregation. Pearson receives an analogous secondary study-balanced summary. Raw assay values and observations are never pooled.
...
9. A predictor is called the robust overall winner only if it is uniquely first in the overall point estimate, passes the simultaneous all-competitor Spearman gate, remains uniquely first in all four leave-one-study-out analyses, the unique study-balanced Pearson point winner agrees, every overlap-excluded analysis leaves it uniquely first, the fixed model-family sensitivity does not change the winner, and all coverage thresholds pass. Any exact tie at one of these required gates is a failure to claim a unique winner. Otherwise report per-dataset/per-study winners and state that no single robust overall winner was established.
...
Exact overlap is normalized-sequence SHA-256 equality. Before overlap code opens either training or evaluation sequence files, freeze and checksum `overlap-audit-spec.json` containing the NCBI BLAST+ and Parasail versions plus the complete BLAST command: task, word size `4`, reward/penalty, gap-open/extension, E-value, both-strand policy, dust and soft-masking disabled, database hash, no target/HSP count truncation, and exact tabular output fields. Generate candidates with that unmasked nucleotide BLAST command, then confirm each candidate with Parasail Smith–Waterman traceback using match `+2`, mismatch `-1`, gap-open `3`, and gap-extension `1`. Identity is exact matches divided by alignment columns including gaps; coverage is aligned non-gap residues divided by the shorter full sequence length. Apply the thresholds above to the confirmed alignment, store coordinates/CIGAR, and test boundary cases and injected positives before accepting the audit. A capped candidate list or an unversioned web service is not allowed.
...
8. **Phase 0 runtime:** before any tool outcome metric is computed, freeze `phase0-selection.json` from the eligibility artifact. Traverse organisms in the fixed order `yeast`, `mouse`, `E. coli`, and within each traverse `S1…S6`. For each cell choose one eligible normalized sequence not previously selected **anywhere in the 18-sequence panel**: nearest to the arithmetic stratum midpoint for `S1…S5`, and longest for open-ended `S6`, with SHA-256 breaking ties. If a cell is empty after global de-duplication, choose the same-organism globally-unused sequence with minimum distance to that stratum's nearest boundary, again breaking ties by SHA-256. If all 18 globally unique hashes cannot be selected, fail Phase 0 rather than duplicate. Record every fallback. For each candidate, collect three application-cache-cold invocations in a hash-seeded balanced order and one warm payload-read measurement with one process and one tool thread. Cold means a fresh dedicated payload-cache root; operating-system page caches are not flushed.
9. **Pilot science:** add 25 deterministic eligible transcripts per dataset, allocating `5,4,4,4,4,4` to `S1…S6` and selecting lowest normalized-sequence SHA-256 values not already in Phase 0. Process initial deficits in source-stratum order `S1…S6`; for each missing slot, scan destination strata cyclically starting immediately after its source (`S(j+1)…S6,S1…Sj`) and take the lowest unused SHA-256 from the first stratum with capacity. Repeat until all deficits are filled; record every reassignment and fail if 25 unique records do not exist. Complete provenance, overlap audit, coordinate inspection, coverage/cohort gates, and all metrics before full scale.
...
- Training overlap is audited for every learned confirmatory candidate, and overlap-excluded sensitivity results are reported.
- Interrupt/resume, failed-attempt retry, concurrent cache access, and worker/input-order invariance are verified.
- A robust-winner claim passes simultaneous family-wise-adjusted superiority against every frozen competitor, unique-first leave-one-study-out and overlap-excluded checks, unique Pearson agreement, fixed model-family, training-overlap, and coverage gates; any required tie fails the claim.
