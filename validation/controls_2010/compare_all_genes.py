...
def read_transcript_blocks(path):
    """Use only deposited exon and UTR boundaries, never inferred alignments."""
    blocks = defaultdict(list)
    with gzip.open(path,'rt') as stream:
        for line in stream:
            name,chrom,start,end,feature = line.rstrip().split('\t')
            if feature in {'Exon','5UTR','3UTR'}:
                blocks[name].append((chrom,min(int(start),int(end)),max(int(start),int(end))))
    return dict(blocks)


def validate_mapping(sequence, location, genome, blocks=()):
    ...
    coordinates = tuple(range(start,end+(1 if start<=end else -1),1 if start<=end else -1))
    if segment.replace('T','U') == sequence.upper().replace('T','U'):
        return coordinates
    # Union the authoritative exon/UTR intervals; joins can represent mature
    # mRNA introns or processed noncoding transcripts such as RDN37.
    retained = set()
    for block_chrom,lo,hi in blocks:
        if block_chrom!=chrom or lo<min(start,end) or hi>max(start,end):
            raise ValueError('Annotated transcript block lies outside its genomic interval')
        retained.update(range(lo,hi+1))
    coordinates = tuple(sorted(retained,reverse=start>end))
    assembled = ''.join(genome[chrom][p-1] for p in coordinates)
    if start>end:
        assembled = assembled.translate(str.maketrans('ACGT','TGCA'))
    if not coordinates or assembled.replace('T','U') != sequence.upper().replace('T','U'):
        raise ValueError('Deposited RNA cannot be reconstructed from its contiguous interval or deposited exon/UTR blocks; no guessed alignment applied')
    return coordinates


def map_measurements(location, wig, opposite_intervals=(), exclude_antisense=True, genomic_positions=None):
    chrom, start, end = location
    ...
    pars = {}
    projection = {g:p for p,g in enumerate(genomic_positions,1)} if genomic_positions is not None else None
    n_ambiguous_measured = 0
    for index in range(bisect_left(coordinates,lo),bisect_right(coordinates,hi)):
        coordinate = coordinates[index]
        if projection is not None and coordinate not in projection:
            continue
        if coordinate in ambiguous:
            n_ambiguous_measured += 1
            if exclude_antisense:
                continue
        pos = projection[coordinate] if projection is not None else (coordinate-start)*direction+1
        pars[pos] = values[index]
...
def parser():
    ...
    p.add_argument('--workers',type=int,default=1,help='Independent transcript processes (Linux); default 1')
...
def process_gene(item):
    """Run the existing single-transcript computation in an isolated process."""
    ...
    number, name = item
    args, output, fingerprint = (_RUNTIME[k] for k in ['args','output','fingerprint'])
    sequences, genome, mapping, wig, intervals, names, tools, inventory, adapters = (
        _RUNTIME[k] for k in ['sequences','genome','mapping','wig','intervals','names','tools','inventory','adapters'])
    ...
    if result is None:
        ...
        try:
            ...
            if name not in mapping:
                ...
                genomic_positions = validate_mapping(sequence.seq,mapping[name],genome,_RUNTIME['blocks'].get(name,()))
                chrom,start,end = mapping[name]
                pars, ambiguous = map_measurements(mapping[name],wig,intervals[(chrom,not(start>end))],not args.keep_antisense_overlaps,genomic_positions)
                result.update(n_measured=len(pars),coverage=len(pars)/len(sequence),n_antisense_measured=ambiguous,
                    ...
                    mapping_mode='contiguous' if len(genomic_positions)==abs(end-start)+1 else 'verified_annotated_exons_and_UTRs')
                if len(pars)<args.min_pairs:
                    ...
                    if args.details:
                        detail_rows = ({'transcript_position':pos,'base':sequence.seq[pos-1],
                            'genome_position':genomic_positions[pos-1],'pars':pars[pos],
...
def main(argv=None):
    ...
    if args.workers>1 and sys.platform!='linux':
        p.error('Multiple workers currently require Linux fork; use --workers 1')
    ...
    config = {k:v for k,v in vars(args).items() if k not in {'resume','retry_failed','source_dir','output_dir','workers'}}
    ...
    if output.exists():
        ...
        dump_json(output/'manifest.json',{'fingerprint':fingerprint,'created_utc':datetime.now(timezone.utc).isoformat(),'config':config,
            'execution_workers':args.workers,'dataset':'GSE22393','source_url':'https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE22393'})
    ...
    blocks = read_transcript_blocks(sources['annotation'])
    ...
    _RUNTIME.update(args=args,output=output,fingerprint=fingerprint,sequences=sequences,
        genome=genome,mapping=mapping,wig=wig,intervals=intervals,names=names,
        tools=tools,inventory=inventory,adapters=adapters,blocks=blocks)
    executor = None
    if args.workers == 1:
        results = map(process_gene, enumerate(names,1))
    else:
        from concurrent.futures import ProcessPoolExecutor
        from multiprocessing import get_context
        # Linux fork shares the immutable genome/WIG arrays between workers.
        executor = ProcessPoolExecutor(max_workers=args.workers,mp_context=get_context('fork'))
        results = executor.map(process_gene,enumerate(names,1),chunksize=1)
    for result in results:
        name = result['transcript_id']
        folder = output/'genes'/hashlib.sha256(name.encode()).hexdigest()[:24]
        if result['fingerprint']!=fingerprint:
            raise ValueError('Checkpoint fingerprint mismatch')
        statuses[result['status']] += 1
        base = {k:result.get(k) for k in ['transcript_id','length','status','n_measured','coverage','n_antisense_measured','mean_pars','mapping_mode','error']}
    ...
    dump_json(output/'summary.json',{'fingerprint':fingerprint,'n_selected_transcripts':len(names),
        ...
        'mapping_mode_counts':dict(Counter({r['transcript_id']:r.get('mapping_mode') for r in rows if r.get('mapping_mode')}.values())),
        'model_status_counts':dict(Counter(r['model_status'] for r in rows if r['model'])),
        'statistics':aggregate})
    columns = ['transcript_id','length','status','model','model_status','n_measured','coverage','n_antisense_measured','mean_pars','mapping_mode','n_pairs','pearson','spearman','error','model_error','report']
...
def gene_report(result):
    lines = [f'# {result["transcript_id"]}\n',f'Status: {result["status"]}. Length: {result["length"]} nt. Measured usable bases: {result.get("n_measured",0)}. Mapping: {result.get("mapping_mode","not established")}.\n']
    if result.get('error'):
